...
router.get('/', async (_req, res) => {
  ...
  const body = `
    <h1>Dashboard</h1>
    <p class="sub">Stock levels and ledger activity across wards, theatres and stores.</p>
...
router.get('/items', async (req, res) => {
  ...
  const body = `
    ...
    <form class="filters" method="get" action="/items">
      ...
      <label>Search<input name="q" value="${esc(search)}" placeholder="e.g. syringe"></label>
